...
public final class AdvancedPerksAPI {
    ...
    private AdvancedPerksAPI() {
        ...
        this.perkRegistry = plugin.getInjector().getInstance(de.fabilucius.advancedperks.registry.PerkRegistry.class);
        this.perkDataRepository = plugin.getInjector().getInstance(de.fabilucius.advancedperks.data.PerkDataRepository.class);
    ...
    public List<Perk> getEnabledPerks(Player player) {
        de.fabilucius.advancedperks.data.PerkData perkData = perkDataRepository.getPerkDataByPlayer(player);
        return new ArrayList<>(perkData.getEnabledPerks());
    }
    ...
    public PerkActionResult enablePerk(Player player, String perkId) {
        Perk perk = getPerk(perkId);
        if (perk == null) return PerkActionResult.PERK_NOT_FOUND;
    ...
    public PerkBuyResult buyPerk(Player player, String perkId) {
        Perk perk = getPerk(perkId);
        if (perk == null) return PerkBuyResult.PERK_NOT_FOUND;
        ...
        switch (result) {
            ...
            case ALREADY_BOUGHT_PERK:
                return PerkBuyResult.ALREADY_BOUGHT;
            case NOT_ENOUGH_FUNDS:
                return PerkBuyResult.NOT_ENOUGH_MONEY;
            case NO_PRICE_SET:
                return PerkBuyResult.NO_PRICE_SET;
            case NO_ECONOMY_INTERFACE:
                return PerkBuyResult.NO_ECONOMY;
            default:
    ...
    private PerkActionResult convertToggleResult(de.fabilucius.advancedperks.data.state.PerkToggleResult result) {
        switch (result) {
            case ENABLED:
                return PerkActionResult.SUCCESS_ENABLED;
            case DISABLED:
                return PerkActionResult.SUCCESS_DISABLED;
            case NO_PERMISSION:
                return PerkActionResult.NO_PERMISSION;
            case TOO_MANY_ACTIVE:
                return PerkActionResult.TOO_MANY_ACTIVE;
            case DISALLOWED_WORLD:
                return PerkActionResult.DISALLOWED_WORLD;
            case EVENT_CANCELLED:
                return PerkActionResult.EVENT_CANCELLED;
            default:
                return PerkActionResult.ERROR;
        }
    }
